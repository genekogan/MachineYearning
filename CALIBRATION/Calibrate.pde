...
void calibrate() {
  A = new Jama.Matrix(projectorPoints.size()*2, 11);
  y = new Jama.Matrix(projectorPoints.size()*2, 1);
  for(int i=0; i < projectorPoints.size()*2; i=i+2){
    PVector kc = robotPoints.get(i/2);
    PVector projC = projectorPoints.get(i/2);
    A.set(i, 0, kc.x);
    A.set(i, 1, kc.y);
    A.set(i, 2, kc.z);
    A.set(i, 3, 1);
  ...
  println("pp points "+ projectorPoints.size() +" pts");
  
  Jama.QRDecomposition problem = new Jama.QRDecomposition(A);
  x = problem.solve(y);
  if (!calibrated) {
    calibrated = true;
  }
